    ...
    return (
      ...
      >
        <div>
          <svg
            style={iconStyle}
            data-icon="symbol-triangle-up"
            width="10"
            height="10"
            viewBox="0 0 16 16"
          >
            <desc>symbol-triangle-up</desc>
            <path
              d="M12.89 11.56l-3.99-8h-.01c-.17-.32-.5-.55-.89-.55s-.72.23-.89.55H7.1l-4 8h.01c-.06.14-.11.29-.11.45 0 .55.45 1 1 1h8c.55 0 1-.45 1-1 0-.16-.05-.31-.11-.45z"
              fillRule="evenodd"
            />
          </svg>
        </div>
  ...
  return (
    ...
    >
      <div>
        <svg
          style={iconStyle}
          data-icon="symbol-triangle-down"
          width="10"
          height="10"
          viewBox="0 0 16 16"
        >
          <desc>symbol-triangle-down</desc>
          <path
  ...
  return (
    <div style={{ margin: '0 12px' }}>
      ...
      >
        <Tooltip
          title={
            <h3 style={{ color: 'white' }}>{getChangeTooltipText(icon)}</h3>
          }
          placement="top"
        >
...
const RenderChangeComponent: React.FC<any> = props => {
  ...
  const percentageText =
    Math.abs(percentage) > 100 ? `100+ %` : `${totalPercentage}%`;
  return (
    <div style={{ display: 'flex', alignItems: 'center', margin: '0 -8px' }}>
      {Object.entries(categoryCountMap).map(
        ([changeCategory, categoryCountDetails]) => (
          <TextWithIcon
            key={changeCategory}
            icon={changeCategory as ChangeTypes}
            text={(categoryCountDetails as CategoryCountDetails).count ?? 0}
          />
        ),
      )}
      <div style={{ margin: '0 12px' }}>

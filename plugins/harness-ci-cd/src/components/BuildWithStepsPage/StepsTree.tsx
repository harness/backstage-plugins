...
import type { ExecutionPipelineNode } from './defs';
  ...
  return (
    <Grid>
      {nodes.map((step, i) => {
        if (step.item) {
          const status: any =
            getStepsTreeStatus({ step, allNodeMap }) || step.item.status;
          const statusLower = status.toLowerCase();

          return (
            <Accordion
              className={pickClassName(classes, statusLower)}
        ...
        if (step.group) {
          const status =
            getStepsTreeStatus({ step, allNodeMap }) || step.group.status;
          const statusLower = status.toLowerCase();

          return (
            <Accordion
              className={pickClassName(classes, statusLower)}
              key={step.group.identifier}
              TransitionProps={{ unmountOnExit: true }}
            >
              <AccordionSummary
                expandIcon={<ExpandMoreIcon />}
        ...
        if (step.parallel) {
          ...
          const isRunning =
            step.parallel.some(pStep =>
              isExecutionRunning(
                defaultTo(pStep.item?.status, pStep.group?.status),
              ),
            ) ||
            step.parallel.some(pStep =>
              isExecutionRunning(
          ...
          if (isRunning) {
            ...
            if (nonSuccessStep) {
              status = defaultTo(
                defaultTo(
                  nonSuccessStep.item?.status,
                  nonSuccessStep.group?.status,
                ),
                '',
              );
            }
